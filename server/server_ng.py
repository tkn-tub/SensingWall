...
while True:
    try:
        ...
        if print_stats_wait_timer.check():
            ...
            rates_list.append({"measurement": "packet_rate", "time": round(time.time() * 1e6), "fields": {"rate": packet_count}})
            packet_count = 0

        if db_write_wait_timer.check():
            t0 = time.perf_counter()
            print(f"[{datetime.now().strftime('%Y-%m-%d %H:%M:%S')}] started writing to influx DB")
            ...
            print("It took", time.perf_counter() - t0, "seconds to write")
